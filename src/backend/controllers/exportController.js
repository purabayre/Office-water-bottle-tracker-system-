...
exports.exportMonthlyReport = async (req, res, next) => {
  try {
    ...
    const summary = memoryStore.isDbConnected(mongoose)
      ? await MonthlySummary.findOne({
          month: m,
          year: y,
        })
      : memoryStore.getSummary(m, y);

    let entries = memoryStore.isDbConnected(mongoose)
      ? await BottleEntry.find({
          month: m,
          year: y,
        }).sort({ date: 1 })
      : memoryStore.getEntriesByMonth(m, y);
